...
import type { Grid } from "@peulicke/geometry/grid";
...
export const pixelsToImage = async (pixels: Pixels) => {
    ...
    canvas.height = pixels[0]?.length ?? 0;
    const ctx = canvas.getContext("2d");
    if (!ctx) return new Image();
    const image = ctx.createImageData(pixels.length, canvas.height);
    ...
    const img = new Image(pixels.length, canvas.height);
    img.src = url;
